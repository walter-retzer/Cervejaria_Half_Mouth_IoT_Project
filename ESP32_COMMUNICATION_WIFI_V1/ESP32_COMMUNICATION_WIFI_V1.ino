/*
   ...
   Comunicação: WiFi - ThingSpeak
   Firmware: V0.0.1
   
   Note: Aumentado filtro de leitura da entrada analógica dos termistores;
         Desabilitado função: adc1_config_channel_atten(ADC1_CHANNEL_6, ADC_ATTEN_DB_0) para comparar o comportamento das temperaturas;
         Desabilitado comunicação ESP-NOW;
         Habilitando comunicação Wifi diretamente para envio dos dados ao ThingSpeak;
*/
...
//========================================================================================================
// Variaveis de Configuração dos Dados Enviados
int32_t data1 = 0;
int32_t data2 = 0;
int32_t data3 = 0;
int32_t data4 = 0;
...
double offsetTemp1 = 3.10;  // valor de offset da temperatura 1
double offsetTemp2 = 3.00;  // valor de offset da temperatura 2
...
int saveSDCardNotOk;
// ========================================================================================================
// Variaveis de Configuração de acesso a Rede WiFi:
...
const char *myWriteAPIKey = "ZZZZZZZZZ";
...
void setup() {
  ...
  Serial.println(F("Iniciando NANO SMART"));
  delay(1000);
...
void loop() {
  ...
  if (millis() - auxTimeSendValue > intervalo) {
    ...
    float field1 = tempCelsius1;
    float field2 = tempCelsius2;
    float field3 = tempCelsius3;
    float field4 = tempCelsius4;
    float field6 = camaraState;
    float field7 = !bombState;
    ...
    writeFileJson(SD, "/send_value.txt", "NANO SMART - SEND VALUE TO THINGSPEAK;");
    auxTimeSendValue = millis();
    auxTimeLoop1 = millis();
    auxTimeLoop2 = millis();
    auxTimeLoop3 = millis();
...
void writeFileJson(fs::FS &fs, const char *path, const char *message) {
  ...
  if (file.print(message)) {
    ...
    file.print(F(";"));
    file.print(F(" TI-001: "));
    file.print(tempCelsius1);
    file.print(F(";"));
    file.print(F(" TI-002: "));
    file.print(tempCelsius2);
    file.print(F(";"));
    file.print(F(" TI-003: "));
    file.print(tempCelsius3);
    file.print(F(";"));
    file.print(F(" TI-004: "));
    file.print(tempCelsius4);
    file.print(F(";"));
    file.print(F(" Bomba: "));
    file.print(bombState);
    file.print(F(";"));
    file.print(F(" Camara Fria: "));
    file.print(camaraState);
    file.print(F(";"));
    file.println(F(" "));
    Serial.println(F("File written"));
    saveSDCardOk++;
...
void displaySetup() {
  ...
  } else if (WiFi.RSSI() < -30 && WiFi.RSSI() >= -55) {
    tft.print(F(" (Otimo)"));
  } else if (WiFi.RSSI() < -55 && WiFi.RSSI() >= -67) {
    tft.print(F(" (Muito Bom)"));
  } else if (WiFi.RSSI() < -67 && WiFi.RSSI() >= -70) {
    tft.print(F(" (Bom)"));
  } else {
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.print(F(" (Fraco)"));
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setFreeFont(FF18);
  tft.print(F("Rede WiFi Conectada: "));
  tft.setFreeFont(FF18);
  if (WiFi.status() == WL_CONNECTED) {
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.print(ssid);
...
/***************************************LOGO_CERVEJARIA******************************************************/
void displaySendValues() {
  tft.fillScreen(TFT_WHITE);
  tft.setRotation(2);
  drawSdJpeg("/server1.jpg", 55, 120);
  tft.setFreeFont(FF44);
  tft.setTextColor(TFT_BLACK, TFT_WHITE);
  tft.setCursor(32, 380);
  tft.print(F("ThingSpeak"));
  tft.setFreeFont(FF34);
  tft.setTextColor(TFT_BLACK, TFT_WHITE);
  tft.setCursor(64, 410);
  tft.print(F(" Enviando os Dados "));
}
/*************************************END_LOGO_CERVEJARIA****************************************************/
...
void displayReport() {
  ...
  bool fluxoState = false;
  tft.setCursor(5, 395);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setFreeFont(FF18);
  tft.print(F("FLUXOSTATO: "));
  tft.setFreeFont(FF18);
  if (fluxoState) {
...
void setupWiFi() {
  ...
  Serial.println("**************************************************************************************************************************************** ");
  Serial.println();
  Serial.print("[WiFi] Conectando Wifi na Rede: ");
  Serial.println(ssid);
  ...
  while (true) {
    ...
    switch (WiFi.status()) {
      case WL_NO_SSID_AVAIL:
        Serial.println("[WiFi] - SSID da Rede Não Encontrado");
        break;
      case WL_CONNECT_FAILED:
        Serial.print("[WiFi] - Falha em Conectar ao WiFi! Motivo: ");
        return;
        break;
      case WL_CONNECTION_LOST:
        Serial.println("[WiFi] - Conexão foi Perdida!");
        break;
      case WL_SCAN_COMPLETED:
        Serial.println("[WiFi] - Scan Realizado");
        break;
      case WL_DISCONNECTED:
        Serial.println("[WiFi] - Rede WiFi esta Desconectada");
        break;
      case WL_CONNECTED:
        Serial.println("[WiFi] - WiFi conectado com Sucesso!");
    ...
    if (numberOfTries <= 0) {
      Serial.print("[WiFi] - Falha em Reconectar com a Rede WiFi!");
      Serial.println(" ");
      Serial.println("**************************************************************************************************************************************** ");
      WiFi.disconnect();
      ESP.restart();
